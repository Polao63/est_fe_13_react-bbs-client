...
export default function Write({ isModifyMode, boardId, handleCancel }) {
  ...
  useEffect(() => {
    if (isModifyMode && boardId) {
      ...
      axios
        .get(`http://34.22.100.101:3000/view?id=${boardId}`)
        .then(response => {
  ...
  const write = e => {
    ...
    axios
      .post("http://34.22.100.101:3000/write", formData, {
  ...
  const update = e => {
    ...
    axios
      .post("http://34.22.100.101:3000/update", formData, {
        headers: { "Content-Type": "multipart/form-data" },
      })
      .then(() => {
        handleCancel();
    ...
    <>
      ...
      <Form onSubmit={isModifyMode ? update : write}>
        ...
        {content.image_path && (
          <div>
            <img
              src={`http://34.22.100.101:3000/${content.image_path}`}
              alt={content.title}
